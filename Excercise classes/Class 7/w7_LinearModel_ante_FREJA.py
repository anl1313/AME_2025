...
def starting_values(y, x):
    ...
    # Return ols estimates in a single array, 
...
def sim_data(N: int, theta, rng):    
    ...
    # Simulate x-values as N(0,1) and add a constant term
    const = np.ones((N, 1))
    x0 = rng.normal(size=(N, K - 1))  # We subtract 1, as the first is constant
