...
def estimate( 
    ...
    t_values = b_hat/se.flatten()
    t_values = b_hat/se.flatten()
    
    names = ['b_hat', 'se', 'sigma2', 't_values', 'R2', 'cov']
    results = [b_hat, se.flatten(), sigma2, t_values, R2, cov]
    return dict(zip(names, results))

    
def est_ols( y: np.ndarray, x: np.ndarray) -> np.ndarray:
    """Estimates y on x by ordinary least squares, returns coefficients
...
def print_table(
    ...
    Args:
        >> labels (tuple): Tuple with first a label for y, and then a list of 
        labels for x.
...
def perm( Q_T: np.ndarray, A: np.ndarray, t=0) -> np.ndarray:
    ...
    # Loop over the individuals, and permute their values.
...
def load_example_data():
    ...
    # Let us also make some variable names
    label_y = 'Log wage'
    label_x = [
        'Constant',
        'Black',
