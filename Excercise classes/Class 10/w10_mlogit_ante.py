...
def loglike(theta, y, x):
    ...
    # denominator 
    denom = np.sum(np.exp(v), axis=1) # Fill in, dont keep dimensions
...
def choice_prob(theta, x):
    """Takes the coefficients and covariates and outputs choice probabilities 
    and log choice probabilities. The utilities are max rescaled before
